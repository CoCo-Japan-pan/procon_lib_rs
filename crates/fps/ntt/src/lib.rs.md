...
data:
  ...
  code: "//! [FFT](https://www.creativ.xyz/fast-fourier-transform/)  \n//! [\u539F\
    ...
    \ x = convolution_ntt_friendly::<MOD1, 3>(\n        a.iter()\n            .map(|x|\
    \ StaticModInt::<MOD1>::new(x.value()))\n            .collect::<Vec<_>>()\n  \
    \          .as_slice(),\n        b.iter()\n            .map(|x| StaticModInt::<MOD1>::new(x.value()))\n\
    \            .collect::<Vec<_>>()\n            .as_slice(),\n    );\n    let y\
    \ = convolution_ntt_friendly::<MOD2, 3>(\n        a.iter()\n            .map(|x|\
    \ StaticModInt::<MOD2>::new(x.value()))\n            .collect::<Vec<_>>()\n  \
    \          .as_slice(),\n        b.iter()\n            .map(|x| StaticModInt::<MOD2>::new(x.value()))\n\
    \            .collect::<Vec<_>>()\n            .as_slice(),\n    );\n    let z\
    \ = convolution_ntt_friendly::<MOD3, 3>(\n        a.iter()\n            .map(|x|\
    \ StaticModInt::<MOD3>::new(x.value()))\n            .collect::<Vec<_>>()\n  \
    \          .as_slice(),\n        b.iter()\n            .map(|x| StaticModInt::<MOD3>::new(x.value()))\n\
    \            .collect::<Vec<_>>()\n            .as_slice(),\n    );\n\n    let\
    \ m1_inv_m2 = StaticModInt::<MOD2>::new(MOD1).inv();\n    let m12_inv_m3 = StaticModInt::<MOD3>::new(MOD1\
    \ as u64 * MOD2 as u64).inv();\n    let m12_mod = M::new(MOD1 as u64 * MOD2 as\
